...
class _PedidosPendientesTiendaState extends State<PedidosPendientesTienda> {
  ...
  final _formKeyApendiente = GlobalKey<FormState>();
  bool iSloadingDelete =false;
  bool iSloadingChange =false;
 bool  iSloadingChangeStatus=false;
  String? selectedStatus;
  List <String> statusList =['Recibido','En preparacion','Listo','Entregado'];
  ...
  Widget build(BuildContext context) {
    ...
    List list = context.watch<GetPedidosProvider>().atStorePendientes;
    ...
    return Scaffold(
        body: SafeArea(child:Column(children: [
          ...
          UsedWidgets().buildListTileForTitle(leadingIcon:Icons.storefront_sharp,
              voidCallback: (){  Navigator.of(context).pushNamed(PedidosPendientesTienda.id);}, title:'Pendientes'  ),
          Divider(thickness: 2,),
                ...
                itemBuilder: (BuildContext context, int index) {return Padding(
                  ...
                  children: [SizedBox(height: 10,),
                          ...
                          UsedWidgets().buildDropDownButtonFixedList(
                              label:'Escoger el status', onChanged: (value) { selectedStatus = value!;}, list:statusList),
                      ...
                      children: [
                        ElevatedButton(
                          ...
                          onPressed: () async{
                            ...
                            if(selectedStatus==null){
                              showstuff(context, 'Por favor, escoger un status');

                            }else{

                              setState(() {iSloadingChange=true;});
                              int orderId = list[index]['id'];
                              await ModifyStatus().modifySatatus(orderId,selectedStatus!);
                              await context.read<GetPedidosProvider>().getAllOrderList();
                              setState(() {iSloadingChange=false;});
                          ...
                          child:iSloadingChange==false? Text('cambiar'):Text('cargando...'),
                        ),
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async{
                            ...
                            await DeleteOrder().deleteOrder(orderId);
                            await context.read<GetPedidosProvider>().getAllOrderList();
                            setState(() {iSloadingDelete=false;});
                          },
                          child:iSloadingDelete==false? Text('Borrar'):Text('cargando...'),
                        ),
                      ],)  ,
                    Divider(thickness: 2,color: Colors.blueGrey,),
                    ...
                    Padding(padding: const EdgeInsets.all(8.0),
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          ...
                          ElevatedButton(style: ElevatedButton.styleFrom(primary: Color(0xff009B77),
                            textStyle: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          onPressed: ()async {
